        ...
        foreach ($children as $key => $post) {
          ...
          if (strtotime('now') >= strtotime($date." America/New_York") && !empty($cost_at_door)) {
            if ($should_show_free_rsvp) {
              ...
              echo '<span>Tickets are free!</span><br /><a class="btn btn-primary js-free-rsvp-btn" data-item-name="'. get_the_title() .'" data-mailchimp-tag-name="' . $mailchimp_tag_name . '" data-item-date="'. $date .'">RSVP</a>';
